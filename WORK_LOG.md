...
- 다중 상태 요청 구현: 단일 기준 또는 1~8방향 × 1~5동작, 상태별 프롬프트/프레임/FPS/loop.
  모든 설정/참조/용량 검증 후 DB transaction으로 등록하며 기존 단일 동작 batch 입력도 지원한다.
- 진행·대기 상한 40개, 순차 실행·개별 취소/실패/결과 채택. 작업 내역은 모든 진행 작업,
  상태 이름·일괄 요청 순번과 이전 완료 기록 더 보기를 제공한다.
- 최대 조합 보관을 위해 프로젝트 128클립/2,048자산 상한을 공용 검증 및 UI에 적용했다.
  64MiB 백업 상한과 원본 부모 관계 검증을 유지한다.
- 프런트엔드 45개, Python 49개 및 TypeScript/Vite 빌드 통과.
  늦은 동작의 잘못된 설정·중복 상태·과용량·DB 실패의 전체 거부, 개별 실패/취소 격리 포함.
- 실제 프런트엔드 요청 생성기 → 별도 API → 로컬 대체 제공자로 40작업을 완료했다.
  80클립/1,280프레임/1,282자산, 약 1.05MB 백업을 만들고 별도 Store에 복원했다.
  PNG 전체 RGBA/크기/부모 관계, 상태·방향·FPS·loop·duration과 ID 재매핑이 일치했다.
- 백업 인코더는 자산 용량을 먼저 확인하고, 누적 PNG만으로 64MiB를 넘으면 다음 이미지의
  디코딩을 중단한다. 최종 JSON의 정확한 UTF-8 크기 검사도 유지하며 조기 중단을 검사했다.
- 별도 QA 화면 127.0.0.1:4186에서 진입 및 결과 작업 내역을 읽기 전용 확인했고 오류 로그는 없었다.
  설정창 열기 자동 승인 검토가 두 차례 시간초과되어 전체 UI 검증은 미완료다.
  허가 질문은 답변 대기이며 같은 클릭을 추가 반복하거나 다른 브라우저 방식으로 우회하지 않는다.
- QA API 8896/Vite 4186과 임시 산출물 `/tmp/sprite-editor-motions-qa-20260915`를 분리했다.
  유료 provider 호출은 없으며, 기존 8796/5186 데이터와 자산을 이 검증에 쓰지 않았다.

## 애니메이션 정렬 완료 (2026-09-15)
