...
## 현재 상태 (2026-09-15)
- 전체 계획은 아직 미완료다. [구현 계획](docs/implementation-plan.md)의 네 단계와
  남은 완료 조건을 기준으로 반복한다. 작은 회귀 수정만 반복하며 전체 범위를 축소하지 않는다.
- 애니메이션 정렬/피벗 구현·검증 완료: 원본 PNG를 보존하며 프레임별 이동량과
  공통 피벗을 프로젝트에 저장한다. 초안 취소, undo/redo, 이전 프레임 겹쳐 보기 지원.
- 미리보기/atlas 공용 배치 계산, 넘친 영역 여백 보존. atlas v2의 원본 사각형·이동량·
  피벗 정보로 원본 크기와 편집 상태를 복원하며 v1 입력은 기존 baked 셀로 유지한다.
- 출력 PNG를 비동기로 인코딩하고 12MiB 상한을 확인한 뒤 다운로드해 입력 용량과 맞춘다.
- 검사 통과: 프런트엔드 40개, Python 45개, TypeScript/Vite 빌드. 공용 fixture로
  출력/입력 계약과 부정확한 피벗·좌표·크기·용량 거부, 후보/프로젝트 ID 재매핑 확인.
- 실제 다운로드 PNG 381×176, 셀 127×176, 피벗 (40,132)의 RGBA 268,224바이트가
  독립 합성과 일치했다. 실제 API 가져오기 후 원본 48×80/96×160/48×80 픽셀,
  125/375/200ms, 이동량·피벗을 보존했다. 재출력 manifest는 새 asset ID 외에 동일하다.
- 브라우저 1280×720/390×844: 정렬 취소·undo/redo, 소수 피벗 입력, 후보 이동/복원,
  one-shot 종료, 새로고침 후 정렬값 복원 확인. 관련 console 오류 미관찰.
  파일 선택창 전체 경로는 기존 장시간 지연 때문에 반복하지 않고 실제 파일을 API로 검증했다.
- 임시 결과와 화면은 `/tmp/sprite-editor-animation-qa-20260915`에 보존한다.
- 다음: 다중 상태 요청, 후보 변형/run 복구, 타일셋·이음새·배경·엔진 출력.
  실제 provider 생성 품질은 실행 허가 대기이며 전체 계획을 완료로 표시하지 않는다.

## 객체 레이어 완료 (2026-09-15)
- 객체 레이어 구현/검증 완료: 타일과 객체의 레이어 종류 분리, 픽셀 좌표 배치·드래그,
  격자 스냅, 피벗·z·개별 충돌 속성, 자산 교체, 더블클릭→Sprite 편집 진입.
- 프로젝트 v3: `objects[]` 및 `layers[].kind`. v1/v2는 기존 레이어를 타일로 변환해
  격자 좌표와 바닥 중앙 정렬을 보존한다. 객체 자산의 검증·백업·복원 ID 재매핑 포함.
- 타일/객체 합계 20,000 상한과 레이어 종류를 검증한다. 객체 드래그 한 번은 undo 한 번이며,
  표시·선택·PNG는 공용 배치 계산을 쓴다. PNG는 참조 이미지 로딩 완료 후 생성한다.
- 검사 통과: 프런트엔드 34개, Python 43개, TypeScript/Vite 빌드.
  기존 타일 383개 보존, 자유 이동 (200,200)→(231,213)→undo/redo,
  격자 스냅·속성·버전 교체, 더블클릭→1px 수정→새 revision 저장→맵 참조 교체 확인.
- 실제 다운로드 PNG 768×512의 RGBA 1,572,864바이트가 독립 합성과 일치했다.
  다운로드 프로젝트의 자산 23개를 별도 Store에 복원하고 픽셀·부모 관계 및
  객체 2개의 좌표/피벗/z/충돌 ID 재매핑, 전체 385개 배치의 동일성을 확인했다.
- 브라우저 localhost:5186, 1280×720 및 390×844에서 입력·스크롤·undo 확인.
  새로고침 후 객체와 수정본 유지, 빈 화면/오류 overlay/관련 console 오류 미관찰.
  네이티브 프로젝트 파일 선택창 전체 경로는 장시간 지연 이력 때문에 재실행하지 않았다.
  해당 파일의 실제 검증·가져오기·재매핑 함수를 사용해 복원 계약을 검증했다.
- 임시 검증 산출물은 `/tmp/sprite-editor-object-qa-20260915`에 보존한다.
  QA 자산과 브라우저의 기존 프로젝트, 원본 PNG를 삭제하지 않았다.
- 다음: 애니메이션 피벗/정렬과 출력 왕복, 다중 상태/후보 변형/run 복구,
  타일셋/이음새/반복 배경/shadow/parallax/엔진별 출력. 실제 provider 품질은 실행 허가 대기.
...
- 확인된 방향 기준의 애니메이션 일괄 실행 완료: 수동 기준 매핑 또는 방향 job 결과로
  시작하며 이미지 변경 시 검토 체크 해제. 선택된 1~8방향만 한 동작으로 요청.
  모든 참조/설정/용량 검증과 DB transaction으로 부분 등록 방지.
  방향별 독립 job의 실패·취소·완료를 유지하고 클립 이름에 방향을 기록.
  로컬 대체 생성기의 실패 후 다음 방향 성공 및 HTTP 왕복 검사, 모바일/데스크톱 UI 확인.
...
- 프로젝트 백업 검증 보강: 가져오기와 동일한 64MiB UTF-8 파일 크기 상한을
  내보내기에 적용하고 portable 구조/자산 참조 검증 후 다운로드. PNG 순차 변환.
  정상 JSON/375ms 보존, 누락 참조 거부, 문자열 길이보다 큰 UTF-8 바이트 수 검사.
  프런트26 및 빌드 통과. 화면 구성 변경 없음; 이번 단위의 브라우저 다운로드 재검사는 미실행.
